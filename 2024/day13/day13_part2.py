...
def _find_lowest_score(button_a: Button, button_b: Button, prize: tuple[int, int]):
    ...
    # Determine which button is the less costly to press.
    button_a_value = max(desired_x / button_a.offset_x / button_a.cost, desired_y / button_a.offset_y, button_a.cost)
    button_b_value = max(desired_x / button_b.offset_x / button_b.cost, desired_y / button_b.offset_y, button_b.cost)

    start_button_a = 0
    start_button_b = 0
    if button_a_value < button_b_value:
        start_button_a = min(desired_x // button_a.offset_x, desired_y // button_a.offset_y)
        print(f"Better to press A, starting A at {start_button_a}")

    else:
        start_button_b = min(desired_y // button_b.offset_x, desired_y // button_b.offset_y)
        print(f"Better to press B, starting B at {start_button_b}")


    for num_button_a in itertools.count(start_button_a):
        for num_button_b in itertools.count(start_button_b):
            ...
            if x == desired_x and y == desired_y:
                ...
                if lowest_score == 0 or cost < lowest_score:
                    lowest_score = cost
        x = button_a.offset_x * num_button_a
        y = button_a.offset_y * num_button_a
        if x > desired_x or y > desired_y:
            break
    return lowest_score
